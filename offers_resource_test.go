...
func TestOfferResourceMarshallJSON(t *testing.T) {
	testdata := testutil.ReadFileBytes(t, filepath.Join("./testdata", "offers.json"))
	...
	for i, test := range tests {
		...
		t.Run(fmt.Sprintf("%d", i), func(t *testing.T) {
			var resource interstellar.OfferResource
			if err := (&resource).UnmarshalJSON([]byte(test)); err != nil {
				t.Fatal(err)
			}
			...
			var resource2 interstellar.OfferResource
			if err := (&resource2).UnmarshalJSON(data); err != nil {
				t.Fatal(err)
			}
